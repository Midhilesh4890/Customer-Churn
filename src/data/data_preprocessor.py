...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cleaned_df["SeniorCitizen"] = cleaned_df["SeniorCitizen"].map(
        {0: "No", 1: "Yes"})
...
def preprocess_data(
    ...
    for i, category in enumerate(categorical_features):
        feature_names.extend([
            f"{category}_{val}" for val in ohe.categories_[i][1:]
        ])

    # Convert to DataFrame for easier handling
    X_train_processed = pd.DataFrame(
        X_train_processed.toarray() if hasattr(
            X_train_processed, 'toarray') else X_train_processed,
        columns=feature_names
    )

    X_test_processed = pd.DataFrame(
        X_test_processed.toarray() if hasattr(
